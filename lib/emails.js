...
function formatPlainTextSection(header, items, formatItem, isAdhoc = false) {
  if (items.length === 0) {
    return "";
  }
...
const formatReviewItem = ({ title, creator, lastModifier, status, conceptStatus, ipdcCompareUrl }) =>
  `   • ${title}\n` +
  `     - Aangemaakt door: ${creator}\n` +
  `     - Laatst bewerkt door: ${lastModifier}\n` +
  `     - Status: ${status}\n` +
