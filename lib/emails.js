...
export function generatePlainTextSummaryEmail(
  ...
  if(isSingular) {
    email += `Een instantie in je LPDC-omgeving (${LPDC_URL}) vraagt jouw aandacht / een actie:\n\n`;
  } else {
    email += `Een aantal instanties in je LPDC-omgeving (${LPDC_URL}) vragen je aandacht / een actie:\n\n`;
  }

  const feedbackHeader = isAdhoc ? "Actie (feedback)" : "Feedback";
  const reviewHeader = isAdhoc ? "Actie (herziening)" : "Herziening";
  const formalHeader = isAdhoc ? "Actie (omzetting u/je)" : "Omzetting naar de 'je'-vorm";
