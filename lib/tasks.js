import { uuid } from "mu";
import { getUUIDFromUri, getWindowStart, formatDate, stripHtmlAndTruncate } from "../utils/utils";
import {
      ...
      .map(
        ({
          ...
          `<li style="margin-bottom: 8px;"><strong>${title}</strong></li>
           ...
           <li style="margin-bottom: 8px;">Feedback: ${stripHtmlAndTruncate(feedbackText, 100)} - ${feedbackOrganization} - ${formatDate(new Date(feedbackDate))}</li>
          `,
      )
      .join("");
    htmlFeedbackRows += `</ul>`;
