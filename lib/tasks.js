...
export async function processNotifications(frequency) {
  ...
  for (const subscription of subscriptions) {
    ...
    if (error) {
      errors.push(error);
      console.error(`[${frequency}] Failed to process subscription ${subscription.uri}:`, error);
    }
  }

  if (errors.length > 0) {
    console.error(`[${frequency}] Completed with ${errors.length} error(s) out of ${subscriptions.length} subscription(s)`);
  }
}
