...
export async function processNotifications() {
  ...
  if (isStatusReportDue()) {
    ...
    if (!ENABLE_STATUSREPORT_NOTIFICATIONS) {
      ...
      try {
        bestuurseenheden = await getAllBestuurseenheden();
      } catch (error) {
        statusReportHasFailure = true;
        bestuurseenheden = [];
      }

      const runStartedAt = getStatusReportRunStart();
      ...
      for (const b of bestuurseenheden) {
        ...
        try {
          ...
          console.log(`Error checking existing status report for ${b.bestuurseenheid}: ${err}`);
          // fall through and reprocess rather than silently skip on a check failure
        }

        if (alreadySent) {
          console.log(`Skipping ${b.bestuurseenheid}, already sent since ${runStartedAt}.`);
          continue;
        }
...
async function processStatusReport(recipient, job) {
  ...
  try {
    ...
    if (!statusReportData || statusReportData.totalInstances === 0) {
