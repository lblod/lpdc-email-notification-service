...
export function generateHtmlSummaryEmail(
  ...
  if (feedbackInstances.length > 0) {
    htmlFeedbackRows += `<ul style="margin-top: 5px; padding-left: 0; list-style-type: none;">`;
  ...
  if (formalInformalInstances.length > 0) {
    htmlFormalInformalRows += `<ul style="margin-top: 5px; padding-left: 0; list-style-type: none;">`;
    htmlFormalInformalRows += formalInformalInstances
      .sort((a, b) => new Date(b.formalInformalModifiedDate) - new Date(a.formalInformalModifiedDate))
      .slice(0, 7)
      .map(({ title, creator, lastModifier, dutchLanguageVariant }) =>
        `<li style="margin-bottom: 24px;">
           <strong style="display: inline-block; margin-bottom: 4px;">${title}</strong><br />
           <span>Aangemaakt door ${creator}</span><br />
           <span>Laatst bewerkt door ${lastModifier}</span><br />
           <span>Taalvariant: ${dutchLanguageVariant}</span>
        </li>`
      )
      .join("");
...
export async function processNotifications(frequency) {
  const subscriptions = await getActiveNotificationPreferences(frequency);
  const errors = [];
  ...
  for (const subscription of subscriptions) {
    ...
    if (error) {
      errors.push(error);
      console.error(
        `[${frequency}] Failed to process subscription ${subscription.uri}:`,
        error,
      );
    }
  }
  ...
  if (errors.length > 0) {
    console.error(
      `[${frequency}] Completed with ${errors.length} error(s) out of ${subscriptions.length} subscription(s)`,
    );
  }
}
...
async function processSubscription(subscription) {
  ...
  // Skip if already notified within the current window
  if (subscription.lastNotifiedAt && subscription.lastNotifiedAt >= windowStart) {
    return result;
  }

  try {
    const feedbackInstances = subscription.notifyFeedback
      ? await getFeedbackChanges(subscription.instanceUris, since, subscription.orgUuid)
      : [];

    const reviewInstances = subscription.notifyReviewNeeded
      ? await getReviewStatusChanges(subscription.instanceUris, since, subscription.orgUuid)
      : [];

    const formalInformalInstances = subscription.notifyFormalInformal
      ? await getFormalInformalChanges(subscription.instanceUris, since, subscription.orgUuid)
      : [];
    ...
    const errMsg = `Error processing subscription ${subscription.uri}: ${err.message || err}`;
