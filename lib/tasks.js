 ...
 */
export function generateStatusReportPlainTextEmail(targetLabel, bestuurseenheid, statusReportData) {
  let email = `Beste ${targetLabel},\n\n`;

  email += `Bij deze het halfjaarlijkse statusrapport voor bestuurseenheid ${bestuurseenheid}.\n\n`;

  email += `Bestuurseenheid ${bestuurseenheid} heeft momenteel ${statusReportData.totalInstances} instanties. Daarvan:\n\n`;
  email += `- hebben ${statusReportData.totalHerziening} een herziening nodig. Bekijk de instanties via ${LPDC_URL}/?isReviewRequiredFilterEnabled=true\n`;
  email += `- hebben ${statusReportData.totalFeedback} openstaande feedback. Bekijk de instanties via ${LPDC_URL}/?isFeedbackAvailable=true\n`;
  email += `- moeten ${statusReportData.totalFormalInformal} nog worden omgezet naar de 'je'-vorm. Bekijk de instanties via ${LPDC_URL}/?needsConversionFromFormalToInformalFilterEnabled=true\n\n`;
 ...
 */
export function generateStatusReportHtmlEmail(targetLabel, bestuurseenheid, statusReportData) {
  return `
  <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
  <html xmlns="http://www.w3.org/1999/xhtml" lang="nl">
  <head>
  ...
  <body>
    ...
    <p style="margin:0 0 16px 0; font-family:sans-serif; letter-spacing:normal; line-height:1.6">
      Bij deze het halfjaarlijkse statusrapport voor bestuurseenheid ${bestuurseenheid}.
    </p>

    <p style="margin:0; font-family:sans-serif; letter-spacing:normal; line-height:1.6">
      Bestuurseenheid ${bestuurseenheid} heeft momenteel <strong>${statusReportData.totalInstances}</strong> instanties. Daarvan:
    </p>

    <ul>
      <li style="margin-bottom: 15px;">hebben <strong>${statusReportData.totalHerziening}</strong> een herziening nodig. Bekijk de instanties via 
      <a href="${LPDC_URL}/?isReviewRequiredFilterEnabled=true">deze link</a>. </li>
      <li style="margin-bottom: 15px;">hebben <strong>${statusReportData.totalFeedback}</strong> openstaande feedback. Bekijk de instanties via 
      <a href="${LPDC_URL}/?isFeedbackAvailable=true">deze link</a>. </li>
      <li style="margin-bottom: 15px;">moeten <strong>${statusReportData.totalFormalInformal}</strong> nog worden omgezet naar de 'je'-vorm. Bekijk de instanties via 
      <a href="${LPDC_URL}/?needsConversionFromFormalToInformalFilterEnabled=true">deze link</a>. </li>
    </ul>
...
function createStatusReportEmail(targetLabel, emailAddress, bestuurseenheid, statusReportData) {
  const subject = `Halfjaarlijkse statusrapport ${bestuurseenheid}`;
