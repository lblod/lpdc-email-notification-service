...
- Feedback: ${statusReportData.totalFeedback} instanties hebben feedback gekregen. Bekijk deze instanties via ${LPDC_URL}/?isFeedbackAvailable=true
    ...
    Wat moet je doen?
    Handleiding: https://abb-vlaanderen.gitbook.io/informatie-lpdc/handleiding-lpdc-module-in-loket-lokale-besturen/feedback-op-een-instantie
...
- Herziening: ${statusReportData.totalHerziening} instanties zijn gekoppeld met een gewijzigd concept. Bekijk deze instanties via ${LPDC_URL}/?isReviewRequiredFilterEnabled=true
    ...
    Wat moet je doen?
    Handleiding: https://abb-vlaanderen.gitbook.io/informatie-lpdc/handleiding-lpdc-module-in-loket-lokale-besturen/herziening-nodig-wijziging-aan-een-concept-verwerken-in-een-instantie

- Dubbele instanties: Van ${statusReportData.totalDuplicateProductIds} instanties bestaan er meerdere versies.
    ...
    Wat moet je doen?
    Handleiding: https://abb-vlaanderen.gitbook.io/informatie-lpdc/handleiding-lpdc-module-in-loket-lokale-besturen/product-of-dienst-verwijderen
`;
...
- U > Je-omzetting: ${statusReportData.totalFormalInformal} instanties moeten nog worden omgezet. Bekijk de instanties via ${LPDC_URL}/?needsConversionFromFormalToInformalFilterEnabled=true
    ...
    Wat moet je doen?
    Handleiding: https://abb-vlaanderen.gitbook.io/informatie-lpdc/handleiding-lpdc-module-in-loket-lokale-besturen/kies-je-voor-u-of-je
`;
  }
...
export function generateStatusReportHtmlEmail(
  ...
  const formalInformalSection =
    statusReportData.totalFormalInformal > 0
      ? `
        <li style="${paragraphStyle}">
          <strong>U &gt; Je-omzetting:</strong>
          <strong>${statusReportData.totalFormalInformal}</strong> instanties moeten nog worden omgezet naar de je-vorm.
          Bekijk deze
          <a href=${LPDC_URL}/?needsConversionFromFormalToInformalFilterEnabled=true>instanties</a>.
          <br>

          Jouw bestuur heeft gekozen om in de je-vorm te werken. Instanties die aangemaakt zijn voordat deze keuze was gemaakt, hebben de "u"-vorm als basis. Je kan deze instanties eenvoudig omzetten naar de je-vorm.
          <br><br>

          <u>Wat moet je doen?</u><br>
          <a href=${jeVormHandleidingUrl}>Kies je voor "u" of "je"? | Handleiding LPDC</a><br><br>
        </li>
      `
      : '';
  ...
  <body>
    ...
    <p style="${paragraphStyle}">
      Met de <a href=${lpdcInfoUrl}>Lokale Producten- en Dienstencatalogus (LPDC)</a> maak je informatie over de dienstverleningen van jouw bestuur beschikbaar voor én via verschillende kanalen. Toepassingen zoals het Verenigingsloket, de Rechtenverkenner, Mijn Burgerprofiel, Your Europe, … zorgen ervoor dat jouw doelgroep snel en eenvoudig de juiste info én de weg naar jouw dienstverlening vindt.
      <br>
      Tenminste… àls de informatie in LPDC volledig, correct en up-to-date is.
    </p>

    <p style="${paragraphStyle}">
      Dit is de huidige status voor jouw bestuur <strong>${bestuurseenheid}</strong>:
    </p>

    <p style="${paragraphStyle}">
      Jouw LPDC-omgeving bevat <strong>${statusReportData.totalInstances}</strong> instanties.
    </p>

    <ul style="padding-left: 20px; margin: 0;">
      <li style="${paragraphStyle}">
        <strong>Feedback:</strong>
        <strong>${statusReportData.totalFeedback}</strong> instanties hebben feedback gekregen.
        Bekijk deze
        <a href=${LPDC_URL}/?isFeedbackAvailable=true>instanties</a>.
        <br>

        Soms vindt een hergebruiker een typfout, dode link, of verouderde of ontbrekende informatie in een instantie. Dankzij de feedbackfunctie kunnen ze dit nu rechtstreeks melden.
        <br><br>

        <u>Wat moet je doen?</u><br>
        <a href=${feedbackHandleidingUrl}>Feedback op een instantie | Handleiding LPDC</a><br>
      </li>

      <li style="${paragraphStyle}">
        <strong>Herziening:</strong>
        <strong>${statusReportData.totalHerziening}</strong> instanties zijn gekoppeld met een gewijzigd concept.
        Bekijk deze
        <a href=${LPDC_URL}/?isReviewRequiredFilterEnabled=true>instanties</a>.
        <br>
        ...
        <u>Wat moet je doen?</u><br>
        <a href=${herzieningHandleidingUrl}>Herziening nodig: Wijziging aan een concept verwerken in een instantie | Handleiding LPDC</a><br>
      </li>

      <li style="${paragraphStyle}">
        <strong>Dubbele instanties:</strong>
        Van <strong>${statusReportData.totalDuplicateProductIds}</strong> instanties bestaan er meerdere versies.
        <br>

        In de LPDC-omgeving van jouw bestuur staan meerdere versies van hetzelfde concept. Enerzijds betekent dit voor jullie dubbel werk, en anderzijds creëert het verwarring: welke versie is de correcte? Kijk na welke versie je wil houden en verwijder de andere versie(s). Zo heb je slechts één bron van waarheid en moet je ook maar één versie up-to-date houden.<br>
        De concepten met meerdere versies zijn: <i>${statusReportData.duplicateProductTitles.join(", ")}</i>
        <br><br>

        <u>Wat moet je doen?</u><br>
        <a href=${verwijderenHandleidingUrl}>Product of dienst verwijderen | Handleiding LPDC</a><br>
      </li>

        ${formalInformalSection}
    </ul>
