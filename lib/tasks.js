...
import { getUUIDFromUri, getWindowStart } from "../utils/utils";
import { getActiveSubscriptions, getFeedbackChanges, getFormalInformalChanges, getReviewChanges } from "../utils/queries";
import { IPDC_URL, FROM_EMAIL_ADDRESS } from "../env";
...
export function generatePlainTextSummaryEmail(
  ...
  email += `Een aantal instanties in je LPDC-omgeving (https://lpdc.lokaalbestuur.vlaanderen.be/) vragen je aandacht / een actie:\n\n`;
        ...
        .map(
          ({
            ...
            return (
              ...
              `     - Feedback: ${truncatedText} - ${feedbackOrganization} - ${feedbackDate}`
            );
          },
        )
        .join("\n\n") + "\n\n";
  ...
  email += `Ga naar LPDC https://lpdc.lokaalbestuur.vlaanderen.be\n\n`;
...
export function generateHtmlSummaryEmail(
      ...
      .map(
        ({
          ...
          `<li style="margin-bottom: 8px;"><strong>${title}</strong></li>
           ...
           <li style="margin-bottom: 8px;">Laatste bewerkt door ${lastModifier}</li>
           // TODO check appropriate feedback length here
           <li style="margin-bottom: 8px;">Feedback: ${feedbackText.substring(0, 50)}${feedbackText.length > 50 ? "..." : ""} - ${feedbackOrganization} - ${feedbackDate}</li>
          `,
      )
      .join("");
    htmlFeedbackRows += `</ul>`;
      ...
      .map(
        ({
          ...
          return `<li style="margin-bottom: 8px;"><strong>${title}</strong></li>
           <li style="margin-bottom: 8px;">Aangemaakt door${creator}</li>
           <li style="margin-bottom: 8px;">Laatste bewerkt door ${lastModifier} op $${dateModified}</li>
  ...
  <body>
    ...
    <p style="margin:0; font-family:sans-serif; letter-spacing:normal; line-height:1.6">Een aantal instanties in je <a href="https://lpdc.lokaalbestuur.vlaanderen.be/">LPDC-omgeving</a> vragen je aandacht / een actie:</p>
    ...
    <p style="margin:0; font-family:sans-serif; letter-spacing:normal; line-height:1.6">
      <a href="https://lpdc.lokaalbestuur.vlaanderen.be">Ga naar LPDC</a>
    </p>
...
export function processNotifications(cadence) {
  // get the subscriptions for that cadence
  const subscriptions = getActiveSubscriptions(cadence);
  const emails = [];
  const errors = [];

  for (const subscription of subscriptions) {
    const { email, error } = processSubscription(subscription);
...
async function processSubscription(subscription) {
  ...
  try {
    const since =
      subscription.lastNotifiedAt ?? getWindowStart(subscription.cadence);
    ...
    const email = await createEmailForTarget(
